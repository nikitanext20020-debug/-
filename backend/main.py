        ...
        if acc.get('rate_limited_until'):
            try:
                ...
                if limited_until.tzinfo is None:
                    limited_until = limited_until.replace(tzinfo=msk_tz)
...
async def get_discovered_channels(status: Optional[str] = None):
    channels = db.get_found_channels(status=status, only_open_comments=False)
    # Обогащаем каналы информацией о банах по аккаунтам
    try:
        bans_map: dict = {}  # channel -> list of {account_id, phone}
        with db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT cb.channel, cb.account_id, a.phone
                FROM channel_bans cb
                LEFT JOIN accounts a ON a.id = cb.account_id
            ''')
            for row in cursor.fetchall():
                ch = (row['channel'] or '').lstrip('@')
                bans_map.setdefault(ch, []).append({
                    'account_id': row['account_id'],
                    'phone': row['phone'] or str(row['account_id'])
                })
        for ch in channels:
            key = (ch.get('channel') or '').lstrip('@')
            ch['banned_accounts'] = bans_map.get(key, [])
    except Exception:
        for ch in channels:
            ch['banned_accounts'] = []
    return channels
...
async def reset_account_bans(account_id: int):
    ...
    try:
        with db.get_connection() as conn:
            ...
            count = cursor.rowcount
            # Сбрасываем счетчик ошибок, health_status и rate_limited_until
            cursor.execute(
                "UPDATE accounts SET consecutive_errors = 0, health_status = 'healthy', rate_limited_until = NULL WHERE id = ?", 
                (account_id,)
            )
        
        # Если воркер запущен — останавливаем и перезапускаем с чистым state
        was_running = account_id in workers and _worker_is_alive(workers.get(account_id))
        if was_running:
            _stop_worker_for_account(account_id, wait_timeout=10.0)

        # Форсированно чистим реестр — даже если поток ещё технически alive
        with _workers_lock:
            workers.pop(account_id, None)
            _workers_starting.pop(account_id, None)

        if was_running:
            account_data = db.get_account(account_id)
            if account_data:
                _start_worker_for_account(account_data)
