...
from telethon.errors import (
    ...
    PeerFloodError, InviteRequestSentError, UserBannedInChannelError,
)
try:
    ...
    async def _process_channels(self):
        ...
        for name in self.active_channels:
            ...
            try:
                ...
                self.db.mark_post_processed(self.account_id, name, post.id)
                self.db.unlock_channel(name)
                continue  # Продолжаем с другими каналами, не останавливаем аккаунт!
                ...
                # Канал не существует - удаляем из базы принудительно (force=True), НЕ считаем ошибкой аккаунта
