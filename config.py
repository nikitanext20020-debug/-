...
- Будь естественной и эмоциональной, используй эмодзи уместно"""
    ...
    # Лимит слов теперь = «мягкий потолок»: обрезка идёт по границе предложения
    # (см. _finalize_comment), поэтому чуть больший запас не даёт фразам обрываться.
    COMMENT_MAX_WORDS = 18
    # ВАЖНО: gemini-3-flash-preview — это "думающая" (reasoning) модель. Часть
    # токенов уходит на скрытые рассуждения, поэтому при низком лимите модель
    # возвращает пустой ответ или одно обрезанное слово ("Да,", "Ого,").
    # Держим лимит с запасом (reasoning + сам коммент), а длину режем по словам.
    COMMENT_MAX_TOKENS = 800
